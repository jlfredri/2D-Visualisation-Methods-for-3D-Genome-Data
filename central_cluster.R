...
X_i_centre = X_s * new_radii 

# make a matrix containing coordinates and new radii (distances from center)
centre = cbind(X_i_centre, new_radii)
colnames(centre) = c("x", "y", "z", "distance_from_zero")

# save the matrix ("centre") as a .csv file 
write.csv(centre, "/path/to/directory/centre.csv", row.names = TRUE)
