...
X_i_ce = X_s * new_radii 

# make a matrix containing coordinates and new radii (distances from center)
ce = cbind(X_i_ce, new_radii)
colnames(ce) = c("x", "y", "z", "distance_from_zero")

# save the matrix ("ce") as a .csv file 
write.csv(ce, "/path/to/directory/ce.csv", row.names = TRUE)
