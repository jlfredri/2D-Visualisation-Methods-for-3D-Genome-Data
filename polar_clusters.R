...
X_1 = X[,1] * 5
X_2 = X[,-1]
X_poles = cbind(X_1, X_2)

# for y-coordinates: 
# X_3 = X[,2] * 3
# X_4 = X[,-2]
# X_poles = cbind(X_3, X_4)

# for z-coordinates: 
# X_5 = X[,3] * 3
# X_6 = X[,-3]
# X_poles = cbind(X_5, X_6)

## Normalise all distances (radii) to 1
radii = apply(X_poles, 1, function(s) {
  sqrt(sum(s ^ 2))
})
...
X_i_poles = X_s * new_radii 

# make a matrix containing coordinates and new radii (distances from center)
poles = cbind(X_i_poles, new_radii)
colnames(poles) = c("x", "y", "z", "distance_from_zero")

# save the matrix ("poles") as a .csv file 
write.csv(poles, "/path/to/directory/poles.csv", row.names = TRUE)
